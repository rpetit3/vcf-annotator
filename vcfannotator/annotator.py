...
class Annotator(object):
    ...
    def add_annotation_info(self):
        self.__vcf.add_information_fields([
            ...
            ['GI', None, 'String', 'GI number associated with gene'],
            ['GeneName', None, 'String', 'Name of gene'],
            ['GeneDescription', None, 'String', 'Description of gene'],
    ...
    def annotate_vcf_records(self):
        ...
        for record in self.__vcf.records:
            ...
            if len(record.ALT) > 1:
                print 'Multiple SNPs ({0}->{1})\t{2}'.format(
                    record.REF, record.ALT, record_annotation
                )
            elif len(record.ALT[0]) > 1 or len(record.REF) > 1:
                print 'Indel ({0}->{1})\t{2}'.format(
                    record.REF, record.ALT[0], record_annotation
                )
            else:
